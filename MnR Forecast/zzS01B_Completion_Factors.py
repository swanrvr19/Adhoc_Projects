...
# Long format: one row per measure. Every downstream window runs once instead of
# once per measure, which keeps the factor logic in a single place.
...
display(tri_long.limit(10))

print(f"Preview Table: inc_periods")
display(inc_periods.limit(10))
...
calib_periods = (
    ...
    .select(*CLM_SPLIT_KEYS, "fin_inc_month")
)
...
calib_curve = (
    ...
    .filter(F.col("lag_months") <= F.lit(CF_CALIBRATION_LAG_MONTHS))
...
# Every category/measure present in the triangle needs a value, including those with
# no calibration cohort at all.
...
display(tri.filter(F.col("lag_months") > 12).limit(10))
...
def build_level_link_ratios(link_df, level_keys, level_name, apply_credibility):
    ...
    if apply_credibility:
        out = out.filter(
            ...
            & (F.col("ldf_base_amt") >= F.lit(CF_MIN_LINK_BASE_AMT))
        )
    return out
...
# DBTITLE 1,Attach Factors to the Triangle
factors = (
    tri
    .join(
        link_curve.select(
            *CLM_SPLIT_KEYS, "measure", "lag_months",
            ...
            "cdf", "completion_factor_raw", "completion_factor_capped", "completion_factor",
        ),
        on=[*CLM_SPLIT_KEYS, "measure", "lag_months"],
        how="left",
    )
    .withColumn(
        "current_lag_ind", F.col("lag_months") == F.col("current_lag")
    )
...
n_cur = factors.count()
n_cur_distinct = factors.select(*CLM_SPLIT_KEYS, "fin_inc_month", "measure").distinct().count()
...
display(
    factors.groupBy("measure", "link_ratio_source")
...
out_cols = [
    *CLM_SPLIT_KEYS,
    ...
    "completion_factor",       # what S02 divides by
    "is_mature",
    "current_lag",
    "current_lag_ind",
    "ultimate_lag_months",
